#!/usr/bin/python3
import time, random, fcntl, sys, os
...
g) If the point reaches +12 then you will earn a FLAG.
...
token = input("Please submit your team token > ")
if token not in TEAM_TOKEN_LIST:
    print("Invalid token")
    exit(-1)

try:
    lockfile = open(f'/tmp/{token}', 'w')
    fcntl.lockf(lockfile, fcntl.LOCK_EX | fcntl.LOCK_NB)

except:
    print("Sorry, the only SINGLE connection is allowed to each team")
    exit(-1)
    pass

LOADING = "Loading.. just 1/2 minutes.."
print(LOADING)
sys.stdout.flush()
